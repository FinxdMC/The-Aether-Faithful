...
- Improved holy sword, now should one-shot unarmored undead like how it used to behave (and it still should) and made the sword only take only 1 point of damage each hit (it's just not worth it with these nerfs, especially when enchantments such as smite or unbreaking are disabled in a modpack for balance or authentic beta-styled gameplay purposes)
- Removed bronze dungeon surface ruins, they defeat the point of having that square tunnel in the sides of islands and they make it too easy to enter them (didn't bother making it into a config option or a structure piece?)
